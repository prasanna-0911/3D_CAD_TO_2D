"""
3_test_blip2.py - BLIP2 OPT 2.7B Parameter Extraction
======================================================
Extracts 71 parameters using BLIP2 from the target PDF.
"""
...
def check_gpu_and_load():
    """Check GPU and load BLIP2 model."""
    import torch
    ...
    if not get_gpu_memory_info()["available"] or gpu_info["free_gb"] < 5:
        print(f"   [SKIP] Skipping - insufficient GPU memory (need ~5GB)")
        return None, None
    ...
    print("   [OK] BLIP2 loaded")
    
    return model, processor


def extract_with_blip2(model, processor, image):
    """Extract information using BLIP2."""
    from PIL import Image
    
    questions = {
        "dimensions": "List all dimension values with units (mm)",
        "gdt": "What GD&T symbols are present?",
        "title_block": "What is in the title block?",
        "views": "What views are shown?",
        "notes": "What notes are visible?"
    }
...
def save_results(drawing_name: str, extraction: dict, detected_71: dict, detected_gt: dict, output_dir: Path):
    ...
    json_path = output_dir / f"{drawing_name}_blip2_71params.json"
    ...
    excel_path = output_dir / f"{drawing_name}_blip2_71params.xlsx"
...
def process_target_pdf():
    """Process the target PDF with BLIP2."""
    print_header("BLIP2 OPT 2.7B - 71 Parameters Extraction")
    ...
    print("\n[LOADING] Loading BLIP2 model...")
    model, processor = check_gpu_and_load()
    
    if model is None:
        print("\n[WARNING] Skipping BLIP2 due to insufficient GPU memory")
        return
    ...
    print(f"   Extracting with BLIP2...")
    extraction = extract_with_blip2(model, processor, image)
